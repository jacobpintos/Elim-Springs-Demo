...
function replaceStateInDb(newState, removeExtraLogins, label) {
  ...
  return Promise.all([cleanup,wipeCols])
    ...
    .then(function(purge){
      // Left over from the pre-Firebase build; nothing writes it any more, but
      // a device that once ran that version may still be carrying a copy.
...
// ─── LAYOUT MODE ──────────────────────────────────────────────────────────────
// Phone layout or desktop layout. The user-agent check matters as much as the
// width one: a phone held in landscape is wider than 768px but still wants the
// bottom bar, not a sidebar.
function useMobile() {
